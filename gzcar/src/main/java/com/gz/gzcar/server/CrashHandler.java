...
public class CrashHandler implements UncaughtExceptionHandler{
	...
	public static final String TAG = "chenghao";  
	...
	private boolean handleException(Throwable ex) {  
		...
		new Thread() {  
			...
			public void run() {  
				Looper.prepare(); 
				Toast.makeText(mContext, "很抱歉,程序出现异常，工程师会尽快处理.", Toast.LENGTH_SHORT).show();  
				Looper.loop();  
			}  
		}.start();  
		//保存日志文件       
	...
	private String getFilePath() {  
		...
		if (isSDCardExist && isRootDirExist) {  
			file_dir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/GZ_CAR_crashlog/";
		} else {  
			// MyApplication.getInstance().getFilesDir()返回的路劲为/data/data/PACKAGE_NAME/files，其中的包就是我们建立的主Activity所在的包  
			file_dir = MyApplication.getInstance().getFilesDir().getAbsolutePath() + "/GZ_CAR_crashlog/";
		}  
		return file_dir;  
	}  
	/** 
	...
	private String saveCatchInfo2File(Throwable ex) {  
		...
		try {  
			...
			String fileName = "错误信息-crash-" + time + "-" + timestamp + ".txt";  
			String file_dir = getFilePath();  
			Log.i("chenghao", "错误信息＝"+sb.toString());
			if(startsend){
				startsend=false;
			}
